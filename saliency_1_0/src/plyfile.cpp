...
char *type_names[] = {  /* old names of types for backward compatability */
"invalid",
...
void freadWithErrorCheck(int file_type, void *buffer, size_t size, size_t count, FILE *stream)
{
	size_t retVal = 10;
	retVal = fread(buffer, size, count, stream);
	if (retVal < 1)
	{
	...
	if (size > 1)
	{
		if ((file_type == PLY_BINARY_BE && !BIG_ENDIAN)
			|| (file_type == PLY_BINARY_LE && BIG_ENDIAN))
		{
...
void get_binary_item(
  FILE *fp,
  int file_type,	// PLY_BINARY_BE or PLY_BINARY_LE
  int type,
  int *int_val,
  unsigned int *uint_val,
  double *double_val
)
{
	...
	{
	case PLY_CHAR:
		freadWithErrorCheck(file_type, ptr, 1, 1, fp);
		*int_val = *((char *) ptr);
		*uint_val = *int_val;
		*double_val = *int_val;
		break;
	case PLY_UCHAR:
		...
		*double_val = *uint_val;
		break;
	case PLY_SHORT:
		freadWithErrorCheck(file_type, ptr, 2, 1, fp);
		*int_val = *((short int *) ptr);
		*uint_val = *int_val;
		*double_val = *int_val;
		break;
	case PLY_USHORT:
		...
		*double_val = *uint_val;
		break;
	case PLY_INT:
		freadWithErrorCheck(file_type, ptr, 4, 1, fp);
		*int_val = *((int *) ptr);
		*uint_val = *int_val;
		*double_val = *int_val;
		break;
	...
	case PLY_FLOAT:
		...
		*uint_val = *double_val;
		break;
	case PLY_DOUBLE:
		freadWithErrorCheck(file_type, ptr, 8, 1, fp);
		*double_val = *((double *) ptr);
		*int_val = *double_val;
		*uint_val = *double_val;
		break;
